...
### Instruções privadas da instância

O gateway carrega opcionalmente `data/config/INSTRUCTIONS.md` e o injeta em cada
prompt. Esse arquivo contém preferências e procedimentos persistentes da instância,
mas nunca pode ampliar permissões, desativar o sandbox ou substituir o pedido atual,
as regras de segurança deste arquivo ou as skills aplicáveis. O arquivo deve ser
editado pelo configurador local ou por uma operação explicitamente autorizada pelo
proprietário; não permitir edição arbitrária pelo Telegram.
